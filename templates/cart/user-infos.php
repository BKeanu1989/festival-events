        ...
        echo '<div class="woocommerce-custom-fields__field_wrapper">';
            for($i = 0; $i < $quantity; $i++) {
                echo '<p class="form-row form-row-wide validate-required">';
                    echo '<label for="name['.$key.']['.$i.']">' . __('Name', 'festival-events') . ' <abbr class="required" title="'.__('erforderlich', 'festival-events').'">*</abbr></label>';
                    echo '<span class="woocommerce-input-wrapper">';
                        echo '<input type="text" class="input-text" id="name['.$key.']['.$i.']" name="name['.$key.']['.$i.']" required>';
                    echo '</span>';
                echo '</p>';
            }
        echo '</div>';
